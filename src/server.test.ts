...
test("workspace file operation tools are exposed in minimal and full modes only", async (t) => {
  for (const toolMode of ["minimal", "full", "codex"] as const) {
    const context = await fixture(t, { toolMode });
    const tools = await context.client.listTools();
    const byName = new Map(tools.tools.map((tool) => [tool.name, tool]));

    if (toolMode === "codex") {
      for (const name of ["delete", "move", "copy", "rename"]) {
        assert.equal(byName.get(name), undefined);
      }
      continue;
    }

    const deleteTool = byName.get("delete");
    ...
    const deleteProperties = (deleteTool.inputSchema as { properties?: Record<string, unknown> }).properties;
    assert.ok(deleteProperties?.workspaceId);
    assert.ok(deleteProperties?.path);
    assert.ok(deleteProperties?.recursive);

    const moveTool = byName.get("move");
    assert.ok(moveTool);
    assert.equal(moveTool.annotations?.readOnlyHint, false);
    assert.equal(moveTool.annotations?.destructiveHint, true);
    const moveProperties = (moveTool.inputSchema as { properties?: Record<string, unknown> }).properties;
    assert.ok(moveProperties?.workspaceId);
    assert.ok(moveProperties?.source);
    assert.ok(moveProperties?.destination);
    assert.ok(moveProperties?.overwrite);
    assert.ok(moveProperties?.merge);

    const copyTool = byName.get("copy");
    assert.ok(copyTool);
    assert.equal(copyTool.annotations?.readOnlyHint, false);
    assert.equal(copyTool.annotations?.destructiveHint, false);

    const renameTool = byName.get("rename");
    assert.ok(renameTool);
    assert.equal(renameTool.annotations?.readOnlyHint, false);
    assert.equal(renameTool.annotations?.destructiveHint, true);
    const renameProperties = (renameTool.inputSchema as { properties?: Record<string, unknown> }).properties;
    assert.ok(renameProperties?.workspaceId);
    assert.ok(renameProperties?.path);
    assert.ok(renameProperties?.newName);
  }
});
