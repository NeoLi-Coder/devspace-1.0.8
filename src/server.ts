...
function serverInstructions(config: ServerConfig): string {
  ...
  return `Use DevSpace for coding work. Call ${toolNames.openWorkspace} once for each project folder or isolated worktree, then keep using its workspaceId. During continued work in the same project or worktree, do not call ${toolNames.openWorkspace} again. Open another workspace only when changing projects, switching checkout/worktree mode, creating another isolated worktree, or when the current workspaceId is rejected. ${agentsMd}${skills}${inspection}Prefer ${toolNames.edit} for targeted modifications, ${toolNames.write} only for new files or complete rewrites, ${toolNames.delete} for explicit file or directory deletion, ${toolNames.move}/${toolNames.copy}/${toolNames.rename} for workspace-scoped path operations, and ${toolNames.shell} for tests, builds, git inspection, package scripts, and commands that are better executed by the shell. Do not create or modify files with ${toolNames.shell}; avoid shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, or any command whose purpose is to write project files.${artifactInstruction}${showChangesInstruction}`;
}
...
export function createMcpServer(
  ...
  registerAppTool(
    ...
    toolNames.delete,
    {
      title: "Delete path",
      description:
        "Delete a file or directory inside the current workspace. Files and empty directories can be deleted directly; non-empty directories require recursive=true. Absolute paths, workspace escapes, the workspace root, and paths resolving outside the workspace through symlinks or junctions are rejected.",
      inputSchema: {
        workspaceId: z
          .string()
          .describe(workspaceIdDescription),
        path: z
          .string()
          .describe("File or directory path to delete, relative to the workspace root."),
        recursive: z
          .boolean()
          .optional()
          .describe("Required only when deleting a non-empty directory."),
      },
      outputSchema: resultOutputSchema({
        path: z.string(),
    ...
    async ({ workspaceId, ...input }) => {
      ...
      try {
        const deleted = await deleteWorkspacePath(workspace.root, input);
        const result = `Deleted ${deleted.kind} ${input.path}${deleted.recursive ? " recursively" : ""}.`;
        const content = [textBlock(result)];
  ...
  registerAppTool(
    ...
    toolNames.move,
    {
      title: "Move path",
      description:
        "Move a file or directory inside the current workspace. Directory moves support recursive merge with merge=true. Existing file conflicts are rejected unless overwrite=true. Absolute paths, workspace escapes, the workspace root, paths resolving outside the workspace through symlinks or junctions, identical source/destination paths, and moving a directory into itself are rejected.",
      inputSchema: {
        workspaceId: z.string().describe(workspaceIdDescription),
        source: z.string().describe("Source file or directory path, relative to the workspace root."),
        destination: z.string().describe("Destination path, relative to the workspace root."),
        overwrite: z.boolean().optional().describe("Allow existing file conflicts to be replaced. Defaults to false."),
        merge: z.boolean().optional().describe("Allow a source directory to merge into an existing destination directory. Defaults to false."),
      },
      outputSchema: resultOutputSchema({
        source: z.string(),
    ...
    async ({ workspaceId, ...input }) => {
      ...
      const workspace = workspaces.getWorkspace(workspaceId);
      try {
        const moved = await moveWorkspacePath(workspace.root, input);
        const result = `Moved ${moved.kind} ${moved.source} to ${moved.destination}.`;
        const content = [textBlock(result)];
        logToolCall(config, {
          tool: toolNames.move,
          workspaceId,
        ...
        return { content, structuredContent: { result, ...moved } };
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        const content = [textBlock(message)];
        logFailedToolResponse(config, {
  ...
  registerAppTool(
    ...
    toolNames.copy,
    {
      title: "Copy path",
      description:
        "Copy a file or directory inside the current workspace. Directory copies are recursive and support merge=true. Existing file conflicts are rejected unless overwrite=true. Absolute paths, workspace escapes, paths resolving outside the workspace through symlinks or junctions, identical source/destination paths, and copying a directory into itself are rejected.",
      inputSchema: {
        workspaceId: z.string().describe(workspaceIdDescription),
        source: z.string().describe("Source file or directory path, relative to the workspace root."),
        destination: z.string().describe("Destination path, relative to the workspace root."),
    ...
    async ({ workspaceId, ...input }) => {
      ...
      const workspace = workspaces.getWorkspace(workspaceId);
      try {
        const copied = await copyWorkspacePath(workspace.root, input);
        const result = `Copied ${copied.kind} ${copied.source} to ${copied.destination}.`;
        const content = [textBlock(result)];
        logToolCall(config, {
          tool: toolNames.copy,
          workspaceId,
        ...
        return { content, structuredContent: { result, ...copied } };
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        const content = [textBlock(message)];
        logFailedToolResponse(config, {
  ...
  registerAppTool(
    ...
    toolNames.rename,
    {
      title: "Rename path",
      description:
        "Rename a file or directory inside its current parent directory. newName must be a simple name without path separators, traversal, or an absolute path. Existing targets, the workspace root, workspace escapes, and paths resolving outside the workspace through symlinks or junctions are rejected.",
      inputSchema: {
        workspaceId: z.string().describe(workspaceIdDescription),
        path: z.string().describe("File or directory path to rename, relative to the workspace root."),
        newName: z.string().describe("New file or directory name only; path separators and traversal are not allowed."),
      },
      outputSchema: resultOutputSchema({
        source: z.string(),
        destination: z.string(),
    ...
    async ({ workspaceId, ...input }) => {
      ...
      const workspace = workspaces.getWorkspace(workspaceId);
      try {
        const renamed = await renameWorkspacePath(workspace.root, input);
        const result = `Renamed ${renamed.kind} ${renamed.source} to ${renamed.destination}.`;
        const content = [textBlock(result)];
        logToolCall(config, {
        ...
        return { content, structuredContent: { result, ...renamed } };
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        const content = [textBlock(message)];
        logFailedToolResponse(config, {
