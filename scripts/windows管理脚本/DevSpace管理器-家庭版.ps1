...
$edition = '家庭版'
$logPrefix = 'devspace-home'

# 显式环境配置。留空时使用系统环境变量 NODE_DIR、NPM_GLOBAL 或 NPM_CONFIG_PREFIX、GIT_BASH。
$configuredNodeDirectory = 'D:\Program\Portable\devtools\FNM\fnm\aliases\default'
$configuredNpmGlobal = 'D:\Program\Portable\devtools\FNM\npm-global'
$configuredGitBashDirectory = 'D:\Program\Portable\devtools\PortableGit\bin'
...
function Get-DevSpaceSettings {
    ...
    $publicBaseUrl = if ($env:DEVSPACE_PUBLIC_BASE_URL) { $env:DEVSPACE_PUBLIC_BASE_URL } elseif ($config.publicBaseUrl) { $config.publicBaseUrl } else { 'https://home.neoli.cyou' }
    [pscustomobject]@{
        Port          = $port
